...
class MAIN():
    ...
    def colission(self):
        if self.heart.pos == self.zhylan.body[0]:
            ...
            addition = self.heart.weights[random.randint(0,2)]
            # добавляет случайное количество очков от 1 до 3
            while(addition):
                self.score += 1
                addition -= 1
...
while not done:
    ...
    # если прошло 6 секунд то снова заспаунить в случайном месте
    if cur_time - game.heart.time > 6:
        x, y = game.random_location()
        game.heart.random_spawn(x, y)
